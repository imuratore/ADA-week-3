...
```
#### Now using the same tools to sample from the remaining two variables and obtain means and standard deviations, but using a different formula to get poisson-based confidence intervals. The upper bound is obtained by adding the product of 1.96 and the square root of the mean over n to the mean; the lower bound is obtained by subtracting this value from the mean. 
...
```
#### Interestingly, both of the "poisson distributed" variables (zombies killed and years of education) appear fairly normally distributed based on the histograms and qqplots generated from random samples of their values. 
